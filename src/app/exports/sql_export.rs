...
impl fmt::Display for SqlExportError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            SqlExportError::DuplicateColumnName { table, name } => write!(f, "duplicate column name {name} in table {table}"),
            SqlExportError::DuplicateConstraintName { name } => write!(f, "duplicate constraint name: {name}"),
            SqlExportError::EmptyCheckCondition { context } => write!(f, "empty CHECK condition ({context})"),
            SqlExportError::MissingReferencedTable { table, foreign_key } => write!(f, "foreign key {foreign_key} in table {table} references a missing table"),
            SqlExportError::MissingReferencedColumn { table, foreign_key, referenced_table } => write!(f, "foreign key {foreign_key} in table {table} points to a missing column in {referenced_table}"),
            SqlExportError::MissingForeignKeyConstraint { table, column } => write!(f, "column {column} in table {table} is typed as FK but has no matching FK constraint"),
            SqlExportError::AmbiguousForeignKey { table, column } => write!(f, "column {column} in table {table} belongs to multiple FK constraints"),
            SqlExportError::UnsupportedDataType { context, base } => write!(f, "unsupported datatype base {base} ({context})"),
            SqlExportError::IdentifierTooLong { kind, name } => write!(f, "{kind} identifier too long for Oracle: {name}"),
        }
    }
}
    ...
    for (_, table) in sorted_tables(tables) {
        ...
        for (_, attr) in sorted_attrs(table) {
            ...
            if !seen_columns.insert(attr.name.clone()) {
                return Err(SqlExportError::DuplicateColumnName { table: table.title.clone(), name: attr.name.clone() });
            }
        }
    }
...
fn render_domain(
    ...
    for (idx, check) in domain.check_constraints.iter().enumerate() {
        if check.condition.trim().is_empty() {
            return Err(SqlExportError::EmptyCheckCondition { context: format!("domain {}", domain.name) });
        }
...
fn render_table_constraints(
    ...
    if pk_attrs.len() > 1 {
        ...
        add_constraint_name(used_constraints, &name)?;
        lines.push(format!("CONSTRAINT {} PRIMARY KEY ({})", name, join_attr_names(table, &pk_attrs)));
    }
    ...
    for (idx, unique) in table.uniques.iter().enumerate() {
        ...
        add_constraint_name(used_constraints, &name)?;
        lines.push(format!("CONSTRAINT {} UNIQUE ({})", name, join_attr_names(table, &attrs)));
    }
    ...
    for (idx, not_null) in table.not_nulls.iter().enumerate() {
        ...
        lines.push(format!(
            "CONSTRAINT {} CHECK ({})",
            name,
            attrs.iter().map(|id| format!("{} IS NOT NULL", table.attributes[*id].name)).collect::<Vec<_>>().join(" AND ")
        ));
    }
    ...
    for (idx, check) in table.checks.iter().enumerate() {
        ...
        let name = constraint_name_or_fallback(&check.name, &format!("CHK_{}_{}", table.title, idx + 1));
        add_constraint_name(used_constraints, &name)?;
    ...
    for fk in table.fks.values() {
        ...
        for &local_id in &fk.local_attrs {
            let Some((_, ref_id)) = fk_attr_target(table, local_id)? else {
                return Err(SqlExportError::MissingForeignKeyConstraint { table: table.title.clone(), column: table.attributes[local_id].name.clone() });
            };
    ...
    match &attr.attribute_type {
        ...
        AttributeType::ForeignKeyAttribute(_) => {
            ...
            match &referenced_attr.attribute_type {
                AttributeType::Logical(dt) => oracle_type_sql(dt, &format!("foreign key target {}.{}", ref_table.title, referenced_attr.name)),
                AttributeType::Domain(domain_id) => {
                    let Some(domain) = domains.get(*domain_id) else { return Ok("NUMBER".to_string()); };
...
fn resolve_fk_attr<'a>(table: &'a Table, attr_id: AttrId, tables: &'a SlotMap<TableId, Table>) -> Result<(&'a ForeignKey, &'a Table, AttrId), SqlExportError> {
...
fn exact_unique_name(table: &Table, attr_id: AttrId) -> Option<String> {
    table.uniques.iter().find_map(|u| {
        ...
        (attrs.len() == 1 && attrs[0] == attr_id).then(|| {
            constraint_name_or_fallback(&u.name, &format!("UQ_{}_{}", table.title, table.attributes[attr_id].name))
        })
    })
}
...
fn exact_not_null_name(table: &Table, attr_id: AttrId) -> Option<String> {
    table.not_nulls.iter().find_map(|n| {
        ...
        (attrs.len() == 1 && attrs[0] == attr_id).then(|| {
            constraint_name_or_fallback(&n.name, &format!("NN_{}_{}", table.title, table.attributes[attr_id].name))
        })
    })
}
...
fn sorted_attrs(table: &Table) -> Vec<(AttrId, &Attribute)> {
    if table.attr_order.is_empty() {
        ...
        table.attr_order.iter().filter_map(|&id| table.attributes.get(id).map(|attr| (id, attr))).collect()
    }
}
...
fn sorted_attr_ids(table: &Table, set: &HashSet<AttrId>) -> Vec<AttrId> {
    if table.attr_order.is_empty() {
        ...
        table.attr_order.iter().copied().filter(|id| set.contains(id)).collect()
    }
}

fn join_attr_names(table: &Table, attrs: &[AttrId]) -> String {
    attrs.iter().map(|id| table.attributes[*id].name.clone()).collect::<Vec<_>>().join(", ")
}

fn ensure_name(kind: &'static str, name: &str) -> Result<(), SqlExportError> {
    if name.trim().is_empty() { return Err(SqlExportError::EmptyName { kind }); }
    if name.len() > IDENTIFIER_LIMIT { return Err(SqlExportError::IdentifierTooLong { kind, name: name.to_string() }); }
    Ok(())
}

fn add_constraint_name(used: &mut HashSet<String>, name: &str) -> Result<(), SqlExportError> {
    if name.len() > IDENTIFIER_LIMIT { return Err(SqlExportError::IdentifierTooLong { kind: "constraint", name: name.to_string() }); }
    if !used.insert(name.to_string()) { return Err(SqlExportError::DuplicateConstraintName { name: name.to_string() }); }
    Ok(())
}
